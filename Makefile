...
INTEGRATION := custom_components/hassd_relay
...
$(DEV_CONFIG)/custom_components/hassd_relay: | $(DEV_CONFIG)/custom_components
	@ln -sfn $(CURDIR)/$(INTEGRATION) $@

run: install $(DEV_CONFIG)/custom_components/hassd_relay ## Launch a local HA with this integration symlinked
	$(HASS) -c $(DEV_CONFIG)
